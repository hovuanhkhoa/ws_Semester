...
class UrlGeneratorTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $routes = $this->getRoutes('test', new Route('/testing/{pages}'));
        $url = $this->getGenerator($routes)->generate('test', array('pages' => 0), UrlGeneratorInterface::ABSOLUTE_PATH);
    ...
    {
        $routes = $this->getRoutes('test', new Route('/{slug}/{pages}', array('slug' => 'index', 'pages' => 0)));
        $this->assertSame('/app.php/index/1', $this->getGenerator($routes)->generate('test', array('pages' => 1)));
    ...
    {
        $routes = $this->getRoutes('test', new Route('/test', array('pages' => 1)));

        $this->assertSame('/app.php/test?pages=2', $this->getGenerator($routes)->generate('test', array('pages' => 2)));
        $this->assertSame('/app.php/test', $this->getGenerator($routes)->generate('test', array('pages' => 1)));
        $this->assertSame('/app.php/test', $this->getGenerator($routes)->generate('test', array('pages' => '1')));
    ...
    {
        $routes = $this->getRoutes('test', new Route('/{pages}.{_format}'));
        $generator = $this->getGenerator($routes);

        $this->assertSame('/app.php/index.mobile.html', $generator->generate('test', array('pages' => 'index', '_format' => 'mobile.html')));
    }
    ...
    {
        $routes = $this->getRoutes('test', new Route('/{pages}.{_format}'));
        $this->getGenerator($routes)->generate('test', array('pages' => 'index', '_format' => 'sl/ash'));
    }
    ...
    {
        $routes = $this->getRoutes('test', new Route('/{pages}.{_format}'));
        $this->getGenerator($routes)->generate('test', array('pages' => 'do.t', '_format' => 'html'));
    }
    ...
    public function testGenerateRelativePath()
    {
        ...
        $this->assertSame('comments?pages=2', $generator->generate('comments',
            array('author' => 'fabien', 'article' => 'symfony-is-great', 'pages' => 2), UrlGeneratorInterface::RELATIVE_PATH)
        );
